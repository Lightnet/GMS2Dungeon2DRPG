...
if(array_height_2d(traps) <= list_max){
    list_index = 0;
}else{
    if(list_index > array_height_2d(traps) - list_max){
        list_index = array_height_2d(traps) - list_max;
    }
}
//draw iamge and check if mouse is press left
for (i = list_index; i < array_height_2d(traps); i++){    
    if(list_count >= list_max){
        break;
    }
    if(screenx > 4 and screenx < (32+4) and screeny > (yy - 64 + (32+4)*list_count) and screeny < (yy - 32 + (32+4)*list_count)){    
    }
    draw_text(48,yy+((32+4)*list_count)-64,traps[ i, 0]);
    if(traps[ i, 1] != noone){
        var xOffset = sprite_get_xoffset(traps[ i, 1]);
        var yOffset = sprite_get_yoffset(traps[ i, 1]);
        draw_sprite(traps[ i, 1],0,4+xOffset,yy+((32+4)*list_count)-64+yOffset);
    }else{
