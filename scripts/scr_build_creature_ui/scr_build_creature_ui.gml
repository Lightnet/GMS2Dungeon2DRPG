...
for (i = list_index; i < array_height_2d(creatures); i++){    
    ...
    draw_text(48,yy+((32+4)*list_count)-64,creatures[ i, 0]);
    if(creatures[ i, 1] != noone){
        var xOffset = sprite_get_xoffset(creatures[ i, 1]);
        var yOffset = sprite_get_yoffset(creatures[ i, 1]);
        draw_sprite(creatures[ i, 1],0,4+xOffset,yy+((32+4)*list_count)-64+yOffset);
    }else{
